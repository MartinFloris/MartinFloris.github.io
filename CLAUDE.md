...
Individual project pages live in `collections/projectXX-slug.html` and are hand-authored (not generated) — see `.github/copilot-instructions.md` for the required per-project template (GA4 script, breadcrumb, `.properties`/`.property-row` metadata block, `../` relative paths back to root). `scripts/update_collection_metadata.py` only patches in the meta/OG/JSON-LD block, it doesn't create the page.
