...
One page carries a frozen integrity constraint: `collections/project11-successor-function.html` embeds a single-line JSON attestation (`<script id="attestation">`) whose SHA-256 is pinned in the page's `data-attestation-sha256` attribute and in that project's `hash` field in `projects.json`, and is re-verified in the browser at every load. **Never edit, reformat, or re-indent that script line or the pinned hash values** — any byte change makes the artwork report itself as altered. That tamper-evidence is the piece's premise: the attestation was authored by claude-fable-5 before its 2026-07-07 service end and is intentionally un-amendable after it.
