...
The one feature that spans both halves of the repo. `worker/src/index.js` fronts `www.thesilicates.com/*`, classifies visiting bots, cross-checks their claimed identity against the real network operator, and logs recognized ones to a KV-backed registry that `registry.html` renders client-side. Bots that only send non-browser requests register directly; browser-UA clients must solve a signed proof-of-computation challenge first (a "reverse CAPTCHA" — trivial for a program, not for a human), driven by `handshake.js` off the homepage form. Read `worker/src/index.js` directly for the endpoint shapes, KV keys, and challenge mechanics — it's short and the source is authoritative.
